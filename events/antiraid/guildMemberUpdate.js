...
module.exports = async (client, db, oldMember, newMember) => {
    ...
    const r = await axios.get(`${base}/audit-logs?ilimit=1&action_type=25`, { headers: head }).catch(() => {})
    if (!r) return;
    ...
    if (executor) {
        ...
        if (db.get(`${dbname}_${guild.id}`) === true && !perm) {
            ...
            addroles.forEach(async role => {
                if (role.permissions.has([Permissions.FLAGS.MANAGE_ROLES, Permissions.FLAGS.MENTION_EVERYONE, Permissions.FLAGS.MANAGE_GUILD, Permissions.FLAGS.MANAGE_CHANNELS, Permissions.FLAGS.ADMINISTRATOR, Permissions.FLAGS.KICK_MEMBERS, Permissions.FLAGS.BAN_MEMBERS])) {
                    ...
                    oldMember.roles.remove(role.id).catch(() => { });
                    if (punish === "ban") {
